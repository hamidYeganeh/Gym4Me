...
- **شواهد پیشرفت محلی (هنوز `BLOCKED`):** mutationهای freeze/unfreeze/transfer/cancel/consume اکنون state + `MembershipEvent` + Outbox را در transaction واحد Mongo ثبت می‌کنند و transfer، holder اصلی را در رکورد immutable قبلی حفظ می‌کند (`apps/api/src/account/memberships/memberships.service.ts`, `apps/api/src/account/memberships/membership-lifecycle-transaction.spec.ts`). فروش عضویت نیز event و Outbox را در transaction فروش ثبت می‌کند (`apps/api/src/account/memberships/application/commands/sell-membership.command.ts`). تمدید همان plan برای پذیرش، preview قیمت/اثر اعتبار با fingerprint، consent نسخه‌دار، idempotency، پرداخت نقد/POS/کارت/ترکیبی، بدهی، Ledger، event و Outbox اتمیک دارد و از UI مالک بدون optimistic update قابل تأیید است (`apps/api/src/account/memberships/application/commands/renew-membership.command.ts`, `apps/mobile/src/modules/owner/screens/OwnerMembersScreen/OwnerMembersScreen.tsx`). خرید و تمدید آنلاین ورزشکار اکنون `MembershipCheckout` پایدار با snapshot مبلغ/credit grant، consent fingerprint، pending `Payment`، آغاز زرین‌پال با callback allowlist، verify بیرون transaction، fulfillment اتمیک membership + Ledger + event + Outbox، idempotent callback و reconciliation دارای lease دارد؛ authority آغازشده بر اثر timeout موقت provider خودکار لغو نمی‌شود و credit grant روی آخرین state جمع می‌شود تا مصرف همزمان از بین نرود (`apps/api/src/schemas/membership-checkout.schema.ts`, `apps/api/src/account/memberships/application/services/membership-checkout.service.ts`, `apps/api/src/finance/finance.service.ts`, `apps/api/src/account/memberships/membership-checkout-reconciliation.worker.ts`, `packages/api/src/memberships/memberships.client.ts`, `apps/mobile/src/modules/discovery/screens/DiscoveryClubsDetailScreen/DiscoveryClubsDetailScreen.tsx`, `apps/mobile/src/modules/athlete/lib/AthleteMembershipsGate.tsx`). خرید اولیهٔ اشتراک پولی owner نیز اکنون `PlatformSubscriptionCheckout` مستقل با snapshot قیمت/دوره، consent، pending Payment، تبدیل صریح IRT→IRR، verify بیرون transaction، fulfillment اتمیک `PlatformSubscription` + Ledger (tax/platform revenue) + Outbox، callback idempotent، reconciliation دارای lease و surface واقعی `/owner/subscription` دارد؛ mutation قدیمی برای طرح پولی fail-closed و فقط برای طرح رایگان idempotent باقی مانده است (`apps/api/src/schemas/platform-subscription-checkout.schema.ts`, `apps/api/src/account/memberships/application/services/platform-subscription-checkout.service.ts`, `apps/api/src/account/memberships/platform-subscription-checkout-reconciliation.worker.ts`, `apps/mobile/src/modules/owner/lib/OwnerPlatformSubscriptionGate.tsx`). boundary فروش همچنان هر `paymentId` خارجی و channel آنلاین ثبت‌نشده را fail-closed می‌کند. callback بومی PSP نیز با broker عمومی و deep link allowlistشدهٔ Capacitor برای booking/wallet/membership/subscription بسته شده است. query ادمین کاتالوگ پلن نیز با collection درست characterization test دارد (`apps/api/src/account/memberships/platform-plan-admin-query.spec.ts`). ADR entitlement snapshot، registry limits، upgrade فوری با proration، downgrade انتهای دوره و grace هفت‌روزه در `docs/product/adr/0001-platform-subscription-entitlements.md` در ۲۰۲۶-۰۸-۲۵ صریحاً `ACCEPTED` شد. checkout واقعی عضویت با mock PSP و fulfillment/Ledger و سناریوی کامل replica-set پاس شد. وضعیت task عمداً `BLOCKED` مانده است: dependency `G4M-021` و `G4M-031` هنوز `VERIFY` هستند؛ اجرای PSP واقعی staging و پیاده‌سازی runtime plan-change/enforcement باز است.
