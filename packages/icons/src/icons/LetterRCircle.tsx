"use client";

import { createIcon } from "../create-icon";

export const LetterRCircle = createIcon(
  "LetterRCircle",
  "0 0 24 24",
  <>
    <circle cx="12" cy="12" r="9" fill="currentColor" fillOpacity="0.2"/>
    <path fillRule="evenodd" clipRule="evenodd" d="M12 6.75C13.7949 6.75 15.25 8.20507 15.25 10C15.25 11.116 14.687 12.0994 13.8301 12.6846C14.9952 13.5502 15.75 14.937 15.75 16.5V17H14.25V16.5C14.25 14.7051 12.7949 13.25 11 13.25H10.7705V17H9.27051V6.75H12ZM10.7705 11.75H12C12.9665 11.75 13.75 10.9665 13.75 10C13.75 9.0335 12.9665 8.25 12 8.25H10.7705V11.75Z" fill="currentColor"/>
    <path fillRule="evenodd" clipRule="evenodd" d="M12 2.25C17.3848 2.25 21.75 6.61522 21.75 12C21.75 17.3848 17.3848 21.75 12 21.75C6.61522 21.75 2.25 17.3848 2.25 12C2.25 6.61522 6.61522 2.25 12 2.25ZM12 3.75C7.44365 3.75 3.75 7.44365 3.75 12C3.75 16.5563 7.44365 20.25 12 20.25C16.5563 20.25 20.25 16.5563 20.25 12C20.25 7.44365 16.5563 3.75 12 3.75Z" fill="currentColor"/>
  </>,
);
