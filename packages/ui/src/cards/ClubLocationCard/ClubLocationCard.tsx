...
function midPoint(route: readonly ClubLocationLatLng[]): ClubLocationLatLng {
  const idx = Math.floor(route.length / 2);
  return route[idx] ?? route[0]!;
}

function toLatLngs(route: readonly ClubLocationLatLng[]): LatLngExpression[] {
...
export function ClubLocationCard({
  ...
  const handleFullscreen = useCallback(
    (e: Parameters<NonNullable<ClubLocationCardProps["onFullscreen"]>>[0]) => {
      fitRoute();
      onFullscreen?.(e);
    },
