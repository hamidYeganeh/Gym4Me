...
export class BookingsService {
  ...
  async create(athleteId: string, dto: CreateBookingDto) {
    ...
    try {
      ...
      const booking = await this.bookingModel.create({
        ...
        status:
          price === 0 ? BookingStatus.CONFIRMED : BookingStatus.AWAITING_PAYMENT,
      });
  ...
  async createClubBooking(athleteId: string, dto: CreateClubBookingDto) {
    ...
    const resolved = await Promise.all(
      dates.map((date) => this.clubSlots.resolveBookableOccurrence(dto.slotId, date)),
    );
  ...
  async listForAthlete(athleteId: string, query: ListBookingsQueryDto) {
    return this.list({ athleteId: new Types.ObjectId(athleteId) }, query, 'athlete');
  }
  ...
  private async rescheduleClubBooking(
    ...
    const targetSlotId =
      dto.slotId ?? booking.resource.refId.toString();
    if (
    ...
    const { slot, occurrence } =
      await this.clubSlots.resolveBookableOccurrence(targetSlotId, dto.date);
  ...
  private async cancel(
    ...
    if (providerCancel || actor === BookingActor.ADMIN) {
  ...
  private async notifyConfirmed(booking: BookingDocument) {
    ...
    await this.notifications.dispatch({
      ...
      params: {
        ...
        date: booking.occurrence?.date ?? booking.startsAt.toISOString().slice(0, 10),
        time: booking.occurrence?.startTime ?? formatTimeTehran(booking.startsAt),
      },
    ...
    return bookings.map((booking) => {
      ...
      const counterpartProjection = counterpart
        ? {
            ...
            avatar: { mediaId: counterpart.avatar?.mediaId?.toString() ?? null },
      ...
      return {
        ...
        slotId:
          booking.slotId?.toString() ?? booking.resource.refId.toString(),
        club: club
          ? {
  ...
  private async resolveClubSlotTargets(
    slotIds: string[],
  ): Promise<Map<string, { title: string | null; coverMediaId: string | null }>> {
    const out = new Map<
      string,
