...
export const athleteSocialPostScreenVariants = tv({
  slots: {
    ...
    commentCard:
      "flex flex-col gap-1 rounded-[20px] border border-border bg-surface p-4",
    compose: "flex flex-col gap-3 rounded-[24px] border border-border bg-surface p-4",
    empty:
      "flex flex-col items-center gap-2 rounded-[24px] border border-border bg-surface px-6 py-8 text-center",
  },
});
