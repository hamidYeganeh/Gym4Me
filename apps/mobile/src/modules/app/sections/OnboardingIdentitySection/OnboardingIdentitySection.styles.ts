...
export const onboardingIdentitySectionVariants = tv({
  slots: {
    root: "flex w-full max-w-lg flex-col gap-6 pb-2",
    title:
    ...
    avatarEdit:
      "absolute end-0 bottom-0 flex size-8 items-center justify-center rounded-full bg-foreground text-background",
    section: "flex flex-col gap-3",
    sectionHead: "flex items-center gap-2",
    sectionIcon: "size-5 text-muted",
    sectionTitle: "text-base font-bold text-foreground",
    field: "w-full",
    input:
      "min-h-12 rounded-2xl border border-border bg-transparent px-4 text-sm shadow-none",
    row: "grid grid-cols-2 gap-3",
    trigger:
      "flex min-h-12 w-full items-center justify-between gap-2 rounded-2xl border border-border bg-transparent px-4 text-start text-sm outline-none data-[hovered=true]:bg-default/40",
    ...
    sliderBlock:
      "flex flex-col gap-2 overflow-hidden rounded-2xl border border-border px-3 py-3",
    security:
      "flex items-start gap-2 text-center text-xs leading-5 text-muted",
    securityIcon: "mt-0.5 size-4 shrink-0",
