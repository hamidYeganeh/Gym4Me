...
import { adminBasics, adminUsers } from "@/shared/lib/api";
import {
  CLUBS_USE_MOCK,
...
const DIRECTIONS: GeoDirection[] = ["north", "south", "east", "west", "center"];
...
export function ClubsCreateForm({
  ...
  >(() => (CLUBS_USE_MOCK ? [...MOCK_CLUB_OWNERS] : []));
  const [categories, setCategories] = useState<
    Array<{ id: string; name: string; slug: string }>
  >(() => (CLUBS_USE_MOCK ? [...MOCK_CLUB_CATEGORIES] : []));
  const [sports, setSports] = useState<Array<{ id: string; name: string }>>(
    () => (CLUBS_USE_MOCK ? [...MOCK_CLUB_SPORTS] : []),
  );
  const isEdit = mode === "edit";
  const createDefaults = CLUBS_USE_MOCK
    ? clubsCreateFormPrefill
    : clubsCreateFormDefaults;
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      if (cancelled) return;
      form.reset(initialValues ?? createDefaults);
      setSubmitError(null);
    });
    return () => {
      cancelled = true;
    };
  }, [createDefaults, form, initialValues]);

  useEffect(() => {
    if (isEdit || CLUBS_USE_MOCK) return;
    let cancelled = false;
    setReferenceError(false);
    void Promise.all([
      adminUsers.list({ role: "club_owner", status: "active", page_size: 200 }),
      adminBasics.listSports({ kind: "category" }),
      adminBasics.listSports({ kind: "sport" }),
    ])
      .then(([ownerPage, categoryPage, sportPage]) => {
        if (cancelled) return;
        setOwners(
          ownerPage.result.map((owner) => ({
            id: owner.id,
            name:
              [owner.name.first, owner.name.last].filter(Boolean).join(" ") ||
              owner.phone,
            phone: owner.phone,
          })),
        );
        ...
        <>
          {CLUBS_USE_MOCK ? (
            <div className="flex items-center justify-between gap-3">
              <Typography className={styles.mockHint()}>
                {t("usingMock")}
              </Typography>
              <Button
                size="sm"
      ...
      <Controller
        ...
        render={({ field }) => (
          <Select
            ...
            onChange={(value) => field.onChange(String(value ?? "standard"))}
          >
