...
export const landingMenuOverlayStyles = tv({
  slots: {
    ...
    top: "flex items-center justify-between px-6 pt-6 text-(--on-brand) sm:px-10 sm:pt-8",
    ...
    link: [
      "block text-4xl font-bold tracking-tight text-(--on-brand)",
      "transition-colors duration-fast ease-app hover:text-accent sm:text-6xl",
    ],
    bottom: [
      "flex flex-col gap-6 border-t border-(--glass-border) px-6 pt-8 pb-6",
